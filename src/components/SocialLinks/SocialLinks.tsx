    ...
    <div className="mt-6 flex items-center justify-center gap-2.5 min-[400px]:gap-3.5 min-[901px]:justify-start min-[901px]:gap-4">
      {socialLinks.map(({ name, href, icon: Icon }) => (
        <a
          ...
          aria-label={name}
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border transition-all duration-250 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--background)] min-[400px]:h-10 min-[400px]:w-10"
          style={{ borderColor: "var(--surface-border)", backgroundColor: "var(--surface-bg)", color: "var(--foreground)" }}
          onMouseEnter={(e) => {
